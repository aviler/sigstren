...
void configRGBLed(void) {
  ...
  conf.master.clk_speed =
      EXAMPLE_I2C_FREQ_HZ; // I2C frequency is the clock speed for a complete
...
void stopRGBLedScript(void) {
  ...
  i2c_master_write_byte(cmd, (EXAMPLE_I2C_ADDR << 1) | EXAMPLE_I2C_WRITE_BIT,
...
void setRGBLedColor(uint8_t color) {
  ...
  case COLOR_RED:
    i2c_master_write_byte(cmd, 'c',
                          EXAMPLE_I2C_ACK_CHECK_EN); // Command fade to color
